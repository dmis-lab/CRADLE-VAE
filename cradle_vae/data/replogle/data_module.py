...
from cradle_vae.data.replogle.download import download_replogle_dataset
...
class ReplogleDataModule(PerturbationDataModule):
    ...
    def _get_cfX(self, adata, X, split):        
        print("get_cfX\tSTART")
        idx_per_t_qc_pass = adata[(adata.obs["split"] == split)&(adata.obs["total_qc"] == 0)].obs.groupby("gene", observed=False).indices
        idx_per_t_qc_fail = adata[(adata.obs["split"] == split)&(adata.obs["total_qc"] == 1)].obs.groupby("gene", observed=False).indices
        ...
        cf_idx = adata[adata.obs["split"] == split].obs.apply(lambda x: np.random.choice(idx_per_t_qc_fail[x.gene], min(len(idx_per_t_qc_fail[x.gene]), 50)), axis=1)
        X = X.cuda()
