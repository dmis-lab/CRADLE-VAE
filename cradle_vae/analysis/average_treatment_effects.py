...
def estimate_model_average_treatment_effect(
    ...
    # _, model_samples, _ = model(
    #     D=dosages_control, qc=quality, condition_values=curr_condition_values, n_particles=1
    # )
    _, model_samples = model(
        D=dosages_control, qc=quality, condition_values=curr_condition_values, n_particles=1
    )
    n_phenos = model_samples["x_good"].shape[-1]
    ...
    for i in range(0, n_particles, batch_size):
        ...
        X_control = model_samples["x_good"].squeeze(1)
        if method == "perturbseq":
        ...
        for t_idx in tqdm(range(dosages_alt.shape[0])):
            ...
            X_curr = model_samples["x_good"].squeeze(1)
            if method == "perturbseq":
                # standardize by library size and log normalize
