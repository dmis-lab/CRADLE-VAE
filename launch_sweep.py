...
# from generate_data import evaluate_wandb_experiment
# from download_ckpt import evaluate_wandb_experiment
...
def launch_sweep(
    ...
    for i in range(num_agents):
        if executor == "sweep_agent":
            ...
            ret = launch_agent_batch_large(sweep_path, host, api_key, agent_idx=i, start_device=start_device, num_agents=num_agents)
        agent_results.append(ret)

    # for i, device in zip(range(num_agents), devices):
    #     if executor == "sweep_agent":
    #         ret = launch_agent_local(sweep_path, host, api_key, agent_idx=i)
    #     elif executor == "sweep_agent_batch":
    #         ret = launch_agent_batch(sweep_path, host, api_key, agent_idx=i)
    #     else:
    #         ret = launch_agent_batch_large(sweep_path, host, api_key, agent_idx=i, device=int(device))
    #     agent_results.append(ret)
...
def launch_agent_batch_large(sweep_path: str, host: str, api_key: str, agent_idx: int, start_device: int, num_agents: int):
    device = agent_idx%num_agents+start_device
    cmd = f"wandb login --host {host} {api_key}; CUDA_VISIBLE_DEVICES={device} wandb agent {sweep_path}"
    return os.system(cmd)
