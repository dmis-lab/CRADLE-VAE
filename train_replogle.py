"""
Training script for CRADLE_VAE models
...
from cradle_vae import data
from cradle_vae.data.utils.perturbation_datamodule import PerturbationDataModule
from cradle_vae.models.utils.lightning_callbacks import (
    GradientNormTracker,
...
def train(config: Dict):
    ...
    trainer = pl.Trainer(
        ...
        devices=config['devices'], # config['devices'], -1
        logger=logger,
        callbacks=callbacks,
    ...
    # TODO: clean up
    val_dataloader = DataLoader(data_module.val_dataset, batch_size=int(data_module.batch_size/4), shuffle=False)
    if lightning_module.predictor is not None:
        val_iwelbo = lightning_module.predictor.compute_predictive_iwelbo(
            val_dataloader, n_particles=100
        )["IWELBO"].mean()
    else:
        val_iwelbo = None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default='./demo/cradle_vae_replogle.yaml', # './demo/cradle_vae_replogle.yaml' , None
    )
